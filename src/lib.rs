...
impl<T> Stream<T> {
    ...
    /// let sink = froop::Stream::sink();
    ///
    /// let folded = sink.stream().fold(40.5, |prev, next| prev + (*next as f32) / 2.0);
    ///
    ...
    /// sink.end();
    ///
    /// assert_eq!(coll.wait(), vec!["yo 41".to_string(), "yo 42".to_string()]);
    /// ```
    ...
    ///
    /// let merged = Stream::merge(vec![sink1.stream(), sink2.stream()]);
    ///
    ...
    /// sink.update(1);
    ///
    /// let coll = rem.collect(); // receives last "remembered" value
    ///
    /// sink.update(2);
    /// sink.end();
    ///
    ...
    /// sink1.end();
    ///
    /// assert_eq!(coll.wait(), vec![(1, "foo"), (2, "foo"), (3, "bar")])
    /// ```
